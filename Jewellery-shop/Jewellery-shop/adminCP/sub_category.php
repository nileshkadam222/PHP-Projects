...
{	
	var falg=0;
	if(document.sub_category.txtPagename.value=="")
	{
		confirm("Enter Sub Category Name");
		falg=1;
	}
	else if(document.sub_category.parentid.value<=0)
	{
		confirm("Select Parent Id ");
		falg=1;
	}
	if(falg==0)
	{
		document.sub_category.frmsub_cat.value=2;
...
{	
	...
	if(falg==0)
	{
		document.sub_category.frmsub_cat.value=1;
		document.sub_category.submit();
	}
}
         ...
         <img src="../images/icon_insertfile.gif" width="16" height="16" align="absmiddle" />&nbsp;Insert Sub Category</td>
    	...
    	<td width="100%">
	     ...
	     <? if($_REQUEST['_Pa']=="Delete_scat" and $msg==5) { ?>
           <table width="100%">
           ...
           <? } ?>
         <? if($_REQUEST['_Pa'] != "" and $_REQUEST['_Pa'] !="Delete_scat") {  ?>
         <form name="sub_category" method="POST">
           <input type="hidden" name="frmsub_cat" value="" />
           <? if($_REQUEST['_Pa']=="Edit") { 
		   $updatesql="Select * from sub_category where id=".$_REQUEST['id'];
		   $updateweb=mysql_query($updatesql);
		   $rowupdate=mysql_fetch_array($updateweb); } ?>
		   
           <table cellpadding="0" cellspacing="1" border="0" width="350" class="tborder" >
           <tr>
         ...
         <? if(($_REQUEST['frmsub_cat']==2 or $_REQUEST['frmsub_cat']==1) and ($msg==3 or $msg==4))   { ?>
           ...
           <td colspan="2" class="smalltext"><img src="../images/icon_error.gif" width="16" height="16" align="absmiddle" />&nbsp;&nbsp;<strong><font color="#999999">Error In <? if($msg==3) {echo "Update"; } else { echo "Insert"; } ?></font></strong></td>
            ...
            <tr class="row1">
           		<td width="130">&nbsp;Sub Category Name</td>
                <td><input name="txtPagename" type="text" id="txtPagename" value="<? if($_REQUEST['_Pa']=="Edit") { echo $rowupdate['scate_name']; } ?>" size="30" maxlength="30" /></td>
           </tr>
			<tr class="row2">
              <td>&nbsp;Parent Category</td>
              <td><select name="parentid" style="width:150px; height:22px">
	              <option value="0" >Select Option</option>
                  <?php 
				   $SqlPatenid="select * from category";
         ...
         <br/>&nbsp;
          ...
          <tr>
          <td width="16"><a href="index.php?_Pt=Sub Category" title="Back"><img src="../images/icon_back.gif" border="0" /></a></td>
          <td><span class="smalltext"><a href="index.php?_Pt=Sub Category" title="Back">&nbsp;Back</a></span></td>
          </tr>
          </table>
         <? } ?>
         <? if($_REQUEST['_Pa'] == "" or $_REQUEST['_Pa'] =="Delete_scat") {  ?>
         ...
         <td class="contenttableborder" colspan="2">
                    ...
                    <td class="tabletitlerow" align="" nowrap="nowrap">Sub&nbsp;Category Name</td>
					<td class="tabletitlerow" align="" nowrap="nowrap">Category</td>
                    <td class="tabletitlerow" width="150" align="center" nowrap="nowrap">&nbsp;Options</td>
                   
                    </tr>
					<?  $sqlweb="select * from sub_category order by id";
					...
					if(mysql_num_rows($resultweb) > 0)
					{
                      ...
                      <td colspan="" width="219" align="" valign=""><?=$row_web['scate_name'];?></td>
                      <td colspan="" width="150" align="" valign=""><?=$row_web['parent_id'];?></td>
                      <td colspan="" width="150" align="center" valign="">
                        <a href="index.php?_Pt=Sub Category&_Pa=Edit&id=<?=$row_web['id'];?>&pid=<?=$row_web['parent_id'];?>"> <img src="../images/icon_edit.gif" border="0" />&nbsp;Edit&nbsp;&nbsp;&nbsp;&nbsp;</a>
                      <a href="index.php?_Pt=Sub Category&_Pa=Delete_scat&id=<?=$row_web['id'];?>" onClick="return delete_web()"><img src="../images/icon_delete.gif" border="0" />&nbsp;Delete&nbsp;</a> </td>
