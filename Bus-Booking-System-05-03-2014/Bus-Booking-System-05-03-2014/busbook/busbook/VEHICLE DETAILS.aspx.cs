	...
	public class WebForm2 : System.Web.UI.Page
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			if(!Page.IsPostBack)
			{
				...
				String comStr2="select bname from branch where brid in(select brid from login where staff_id='"+strUserid+"')";
				SqlCommand com=new SqlCommand(comStr2,con);
				SqlDataReader reader=com.ExecuteReader();
				string strBranch="";
				if (reader.Read())
				{
					strBranch=reader["bname"].ToString();
				}
				reader.Close();
				...
				String comstr3="select distinct dated from stafftable where bname='"+strBranch+"'";			
		...
		private void ddldate_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			...
			String comStr="select * from stafftable where bname='"+strBranch+"' and dated='"+strDat+"'";
