...
namespace busbook
{
	...
	public class WebForm3 : System.Web.UI.Page
	{
		...
		protected System.Web.UI.WebControls.TextBox txtvehno;
		...
		{    
			this.txtvehno.TextChanged += new System.EventHandler(this.Calendar1_SelectionChanged);
		...
		private void ddlrname_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			...
			/*The vehicle number for the respective service no bus is
			 extracted from the data base and redirected to the vehno textbox.*/
			String comStr2= "select v.vehno from service s,vehdetails v where s.srno in (select srno from service where routename='"+strRname+"') and s.srno=v.srno";
			SqlCommand com2=new SqlCommand(comStr2,con);
			SqlDataReader dr2=com2.ExecuteReader();
			while(dr2.Read())
			{
				txtvehno.Text=dr2[0].ToString();
			}
			dr2.Close();
		...
		private void Button1_Click(object sender, System.EventArgs e)
		{
			...
			string cmdStr1="select top 1 rid from resdetails order by rid desc";
			...
			string cmdStr="insert into resdetails values("+intId+",'"+strSno+"',"+intResv+",'"+strDat+"','','')";
			Session["sId"]=intId;
			...
			String comStr6="select bname from branch where brid='"+strBrid1+"'";
			...
			string strRname=ddlrname.SelectedItem.Text;
			string cmdStr3="update stafftable set seatsresv="+intResv+",seatsallot="+intSa1+" where bname='"+strBname+"' and dated='"+strDat+"' and routename='"+strRname+"'";
			...
			string cmdStr4="select seatsresv,amount from stafftable where bname='"+strBname+"' and dated='"+strDat+"' and routename='"+strRname+"'";
			SqlCommand cmd4=new SqlCommand(cmdStr4,con);
			SqlDataReader dr7=cmd4.ExecuteReader();
			int intSrsv=0,intAmnt=0;
			if (dr7.Read())
			{
				intSrsv=Convert.ToInt16(dr7[0]);
				intAmnt=Convert.ToInt16(dr7[1]);
				Session["sSrsv"]=intSrsv;
				Session["sAmnt"]=intAmnt;
