...
insert into complaints_linemen_tb(lineman_num,l_name,incentive_earned) values('1000','Nagaraj','0')
insert into complaints_linemen_tb(lineman_num,l_name,incentive_earned) values('1001','Kishore','0')
insert into complaints_linemen_tb(lineman_num,l_name,incentive_earned) values('1002','Jagadish','0')
insert into complaints_linemen_tb(lineman_num,l_name,incentive_earned) values('1003','Pavan','0')
insert into complaints_linemen_tb(lineman_num,l_name,incentive_earned) values('1004','Ram','0')

--select * from complaints_linemen_tb
--update complaints_linemen_tb set status='AVAILABLE' where lineman_num='1001'
...
create table complaints_new_tb(
	...
	consumer_num varchar(7) constraint fk_new_consumers foreign key references 
...
--update complaints_new_tb set falut_at='SUB-STATION' where falut_at='sub-station'
...
create table complaints_assignment_tb(
	...
	lineman_num varchar(4) constraint fk_assignment_linemen foreign key references 
...
--update complaints_assignment_tb set status='ALLOTTED' where ref_num='11'
-----------------------------------------------------------------------------------
select * from complaints_users_tb
select * from complaints_consumers_tb
select * from complaints_linemen_tb
...
select distinct new.ref_num,
	...
	new.nature as [Complaint Nature],new.given_date as [Given Date],new.given_time as [Given Time],
	closed.close_date as [Closed Date],closed.close_time as [Closed Time], 
	assign.lineman_num as [Lineman Number],assign.status as [Complaint Status] ,assign.delay_reason as [Reason for Delay]
...
where 	new.consumer_num=consumers.meter_num and 
	...
	( closed.ref_num=new.ref_num or assign.status='PENDING' or assign.status='ALLOTTED') 
...
where 	new.consumer_num=consumers.meter_num and 
	...
	( closed.ref_num=new.ref_num or assign.status in ('PENDING','ALLOTTED')) 
...
alter procedure complaints_validateDates_proc(@date1 datetime,@date2 datetime,@res int output)as
begin
declare @res1 int
...
--declare @r int 
--execute complaints_validateDates_proc '2007-02-10','2007-02-10',@r output
--select @r
-------------------------------------------------------------------------------------

/* validateDatesWithTimes procedure creation */
alter procedure complaints_validateDatesWithTimes_proc(@date1 datetime,@date2 datetime,@res int output)as
begin
declare @res1 int
...
--declare @r int 
execute complaints_validateDatesWithTimes_proc '2007-02-12 04:30','2007-02-12 05:15',@r output
select @r
--------------------------------------------------------------------------------------
